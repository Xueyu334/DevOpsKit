...
│  ├─ text-diff/            # 文本比对
│  ├─ json-tools/           # JSON 工具
│  ├─ timestamp-converter/   # 时间戳转换 (New!)
│  ├─ password-generator/   # 密码生成器 (New!)
│  ├─ ...                   # 其他工具页面
├─ layout/                  # 响应式布局组件
├─ router/                  # 动态路由配置
├─ App.vue
└─ main.js                  # 入口文件 (包含 Day.js 全局配置)
```
...
- **数据隐私**: 所有转换逻辑（如 Hash、密码）均在浏览器本地完成，**绝不上传**您的任何隐私数据。
- **合规使用**: 身份证生成等工具仅供研发调试使用，严禁用于任何非法途径。

## Roadmap

- [x] 接入 Day.js 提供更强大的全球时区处理能力
- [x] 重构工具分类逻辑，优化 UI 布局与间距
- [ ] 增加 Kubernetes (K8s) 常用命令与资源清单备查
- [ ] 增加正则表达式 (Regex) 在线测试与常用库
- [ ] 增加图片转 Base64 及常用尺寸压制工具
- [ ] 逐步补充 Vitest 自动化单元测试
