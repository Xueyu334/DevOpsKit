...
DevOpsKit 是一个基于 Vue 3 + Vite + Element Plus 构建的前端工具箱，面向研发、测试和运维场景，提供文本处理、编解码、摘要计算和测试数据生成等常用能力。
...
src/
├─ assets/                  # 全局样式和静态资源
├─ composables/             # 可复用逻辑，如 useCopyText
├─ config/
...
- 需要复用的逻辑优先抽到 `composables/`，避免散落在页面中重复实现。
