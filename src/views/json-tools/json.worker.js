...
function renderJSON(obj, depth = 0, options) {
  ...
  if (Array.isArray(obj)) {
    if (obj.length === 0)
      return (
        '<span class="json-bracket">[]</span>' + (showType ? ' <span class="json-type type-array">array</span>' : '')
      )
    ...
    obj.forEach((item, index) => {
      html +=
        '<li>' +
        (showIndex ? '<span style="color:#999; margin-right:5px; font-size:12px">' + index + ':</span>' : '') +
        renderJSON(item, depth + 1, options) +
        (index < obj.length - 1 ? '<span class="json-bracket">,</span>' : '') +
        '</li>'
    })
  ...
  if (typeof obj === 'object') {
    ...
    if (keys.length === 0)
      return (
        '<span class="json-bracket">{}</span>' + (showType ? ' <span class="json-type type-object">object</span>' : '')
      )
    let html = '<span class="json-toggle">-</span><span class="json-bracket">{</span>'
    ...
    keys.forEach((key, index) => {
      ...
      const displayKey = color
        ? '<span class="json-key json-key-' + colorIdx + '">"' + escapeHtml(key) + '"</span>'
        : '"' + escapeHtml(key) + '"'
      html +=
        '<li>' +
        displayKey +
        '<span class="json-bracket">: </span>' +
        renderJSON(obj[key], depth + 1, options) +
        (index < keys.length - 1 ? '<span class="json-bracket">,</span>' : '') +
        '</li>'
    })
    html += '</ul><span class="json-bracket">}</span>'
